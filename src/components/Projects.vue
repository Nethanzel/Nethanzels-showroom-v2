<template>
  <div class="project" id="projects">
    ...
    <section
      class="moreProjects"
      v-if="totalProjects - 3 > 0"
      @click="projectsAll"
    >
      <h4>View more ({{ pluralizeProject(totalProjects - 3) }})</h4>
    </section>
  </div>
</template>
...
export default {
  ...
  methods: {
    getProjects() {
      let root = "/api/projects";
      if(process.env.VUE_APP_DEV) root = new URL(`http://${process.env.VUE_APP_SERVER}/api/projects`);
      axios
        .get(root, { headers: { token: this.client() } })
        .then(res => {
          if (res.status == 200) {
            this.totalProjects = res.data.length;
            this.projects = res.data.reverse();
            this.projects = this.projects.splice(0, 3);
            this.loading = false;
          }
        })
